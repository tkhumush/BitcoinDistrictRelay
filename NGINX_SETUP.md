...
If you have a domain (e.g., relay.bitcoindistrict.org):
...
server {
    listen 80;
    listen [::]:80;
    server_name relay.bitcoindistrict.org;  # CHANGE THIS TO YOUR DOMAIN
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/relay.bitcoindistrict.org/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/relay.bitcoindistrict.org/privkey.pem;
...
Get SSL certificate:
```bash
sudo certbot --nginx -d relay.bitcoindistrict.org
```
...
# Or with domain:
curl https://relay.bitcoindistrict.org -H "Accept: application/nostr+json" | jq
```
...
**With SSL (domain):**
- `wss://relay.bitcoindistrict.org` (via nginx on port 443)
...
```bash
cd /Users/taymurkhumush/Documents/GitHub/bitcoindistrictrelay
nano strfry.conf
```
...
info {
    ...
    contact = "wss://relay.bitcoindistrict.org"  # Or ws://172.105.154.238
}
```
...
- Check: `dig relay.bitcoindistrict.org` or `nslookup relay.bitcoindistrict.org`
...
1. Share your relay URL with users: `ws://172.105.154.238` or `wss://relay.bitcoindistrict.org`
